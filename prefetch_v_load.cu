...
__global__ void prefetch_matmul(const float *__restrict__ a,
  ...
  constexpr int A_SUB_PER_THREAD_LOAD_COUNT =
      M_BLOCK_SIZE * K_BLOCK_SIZE / (NUM_THREADS * 4);
  constexpr int B_SUB_PER_THREAD_LOAD_COUNT =
      K_BLOCK_SIZE * N_BLOCK_SIZE / (NUM_THREADS * 4);
  float a_prefetch_regs[A_SUB_PER_THREAD_LOAD_COUNT * 4] = {0.0f};
  float b_prefetch_regs[B_SUB_PER_THREAD_LOAD_COUNT * 4] = {0.0f};
  ...
  int a_col_index = 4 * tiling_thread_id % K_BLOCK_SIZE;
  int b_col_index = 4 * tiling_thread_id % N_BLOCK_SIZE;

/// Load the first blocked data to shared memory
#pragma unroll
  for (int i = 0; i < A_SUB_PER_THREAD_LOAD_COUNT; ++i) {
    int index_in_block = 4 * i * NUM_THREADS + 4 * tiling_thread_id;
    int row_index = index_in_block / K_BLOCK_SIZE;
    /// Load into registers
    *reinterpret_cast<float4 *>(&a_prefetch_regs[4 * i]) =
        *reinterpret_cast<const float4 *>(
            &a[a_block_begin + row_index * k + a_col_index]);
    /// Transpose
    a_sub[0][a_col_index][row_index] = a_prefetch_regs[4 * i];
    a_sub[0][a_col_index + 1][row_index] = a_prefetch_regs[4 * i + 1];
    a_sub[0][a_col_index + 2][row_index] = a_prefetch_regs[4 * i + 2];
    a_sub[0][a_col_index + 3][row_index] = a_prefetch_regs[4 * i + 3];
  }

#pragma unroll
  for (int i = 0; i < B_SUB_PER_THREAD_LOAD_COUNT; ++i) {
    int index_in_block = 4 * i * NUM_THREADS + 4 * tiling_thread_id;
    int row_index = index_in_block / N_BLOCK_SIZE;
    *reinterpret_cast<float4 *>(&b_sub[0][row_index][b_col_index]) =
        *reinterpret_cast<const float4 *>(
...
#pragma unroll
  for (int i = 0; i < M_THREAD_TILING_SIZE; i += 4) {
    *reinterpret_cast<float4 *>(&a_tiling[0][i]) =
        *reinterpret_cast<float4 *>(&a_sub[0][0][a_tiling_begin + i]);
  }

#pragma unroll
  for (int i = 0; i < N_THREAD_TILING_SIZE; i += 4) {
    *reinterpret_cast<float4 *>(&b_tiling[0][i]) =
        *reinterpret_cast<float4 *>(&b_sub[0][0][b_tiling_begin + i]);
  }
...
#pragma unroll
      for (int i = 0; i < A_SUB_PER_THREAD_LOAD_COUNT; ++i) {
        auto index_in_block = 4 * i * NUM_THREADS + 4 * tiling_thread_id;
...
#pragma unroll
      for (int i = 0; i < B_SUB_PER_THREAD_LOAD_COUNT; ++i) {
        ...
        *reinterpret_cast<float4 *>(&b_prefetch_regs[4 * i]) =
            *reinterpret_cast<const float4 *>(
...
#pragma unroll
      for (int i = 0; i < A_SUB_PER_THREAD_LOAD_COUNT; ++i) {
        ...
        auto row_index = index_in_block / K_BLOCK_SIZE;
        /// Transpose
        a_sub[write_idx][a_col_index][row_index] = a_prefetch_regs[4 * i];
        a_sub[write_idx][a_col_index + 1][row_index] =
            a_prefetch_regs[4 * i + 1];
        a_sub[write_idx][a_col_index + 2][row_index] =
            a_prefetch_regs[4 * i + 2];
        a_sub[write_idx][a_col_index + 3][row_index] =
            a_prefetch_regs[4 * i + 3];
      }
#pragma unroll
      for (int i = 0; i < B_SUB_PER_THREAD_LOAD_COUNT; ++i) {
        auto index_in_block = 4 * i * NUM_THREADS + 4 * tiling_thread_id;
        auto row_index = index_in_block / N_BLOCK_SIZE;
        *reinterpret_cast<float4 *>(&b_sub[write_idx][row_index][b_col_index]) =
            *reinterpret_cast<float4 *>(&b_prefetch_regs[4 * i]);
      }
...
#pragma unroll
    for (int i = 0; i < M_THREAD_TILING_SIZE; i += 4) {
      *reinterpret_cast<float4 *>(&a_tiling[0][i]) =
          *reinterpret_cast<float4 *>(&a_sub[write_idx][0][a_tiling_begin + i]);
    }

#pragma unroll
    for (int i = 0; i < N_THREAD_TILING_SIZE; i += 4) {
      *reinterpret_cast<float4 *>(&b_tiling[0][i]) =
          *reinterpret_cast<float4 *>(&b_sub[write_idx][0][b_tiling_begin + i]);
    }
  }
...
int main() {
  ...
  auto launch_kernel = [](const float *__restrict__ a,
    ...
    gemmCheck(M_THREAD_TILING_SIZE >= 4 && M_THREAD_TILING_SIZE % 4 == 0,
              "M_THREAD_TILING_SIZE must be multiply of 4");
    gemmCheck(N_THREAD_TILING_SIZE >= 4 && N_THREAD_TILING_SIZE % 4 == 0,
              "N_THREAD_TILING_SIZE must be multiply of 4");
    gemmCheck(M_BLOCK_SIZE * N_BLOCK_SIZE / NUM_THREADS % 4 == 0,
              "shared memory size must be 4 times of NUM_THREADS");
